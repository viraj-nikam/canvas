                ...
                <div v-if="isReady">
                    <div v-if="topics.length" class="mt-2">
                        <div v-for="topic in filteredList" class="d-flex border-top py-3 align-items-center">
                            ...
                            <div class="ml-auto">
                                <span class="text-muted mr-3">{{ topic.posts_count }} {{ trans.tags.posts }}</span>
                                <span class="d-none d-md-inline-block">{{ trans.topics.details.created }} {{ moment(topic.created_at).fromNow() }}</span>
                            </div>
                        </div>

                        <div class="d-flex justify-content-center">
                            <a v-if="loadMore" class="btn btn-link text-success text-decoration-none font-weight-bold btn-block" @click.prevent="limit += 10">
